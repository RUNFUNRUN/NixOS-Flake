/*
 *  This file is part of Stream Recorder v1.3.11  <https://www.hlsloader.com/>
 ...
 *
 *  v1.3.11 : Fixed oversights in the interpretation of the MP4 format.
 *  v1.3.10 : Related to Google Chrome's new feature "Memory Saver" : To avoid data loss, we set tab's autoDiscardable property to false.
 *
 *  v1.3.0
...
const intercept_onData = async(tab, params) => {
  ...
  for (const mp4 of mp4s) {
    const trackId = bufferid + (mp4s.length === 1 ? "" : mp4s.indexOf(mp4) % 2);
    video = master.video;
    let duration = 0;
...
const divideByTrack = srcMP4 => {
  ...
  if (src.moov) {
    ...
    if (src.moof) {
      const trafs_length = src.moof.trafs?.length || 0;
      if (trafs_length < 2) {
        return [srcMP4];
      }
      const result = [];
      const wholeData = srcMP4.data;
      const sidx_valid = src.sidxs?.length === src.moofs.length * trafs_length;
      let ttl = 0;
      for (let n = 0; n < src.moofs.length; n++) {
        const moof = src.moofs[n];
        const len = moof.trafs?.length || 0;
        for (let i = 0; i < len; i++) {
          ...
          const sidx = sidx_valid ? ttl : src.sidxs?.length === len ? i : 0;
          if (src.sidx) {
            root.append(src.sidxs[sidx]);
          }
          root.append(new MP4.Box("moof"));
          root.moof.append(moof.mfhd);
          root.moof.append(traf);
          root.arrange();
          ...
          let mdatSize = 0;
          for (const trun of traf.truns || []) {
            let offset = moof.offset + trun.dataOffset;
            let size = 0;
      ...
      return result.length > 0 ? result : [srcMP4];
    }
  }
  return [srcMP4];
};
...
const getMoofDeltaAndDuration = (init, root, index) => {
  ...
  if (index === 0) {
    const dt = moof?.traf?.tfdt?.baseMediaDecodeTime || 0;
    if (dt < -2147483648) {
      moof.traf.tfdt.baseMediaDecodeTime = 0;
    }
  }
  if (sidx) {
...
const onFragParsingData_AV = (video, frag, data) => {
  ...
  if (tracks.length === 1) {
    ...
    for (const mp4 of tracks) {
      ...
      dest.start[pos] = frag.cc * 1000000000 + frag.sn + delta * 0.01;
    }
  }
